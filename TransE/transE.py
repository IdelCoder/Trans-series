    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            if (config.loadFromData):
                saver.restore(sess, './model/transemodel.vec')
            ...
            if not config.testFlag:
                for times in range(config.trainTimes):
                    ...
                    print times
                    print res
                saver.save(sess, './model/transemodel.vec')
                ent_embeddings, rel_embeddings = sess.run([trainModel.ent_embeddings, trainModel.rel_embeddings])
            else:
                if not config.tripleClassification:
                    ...
                    for times in range(total):
                        ...
                        if (times % 50 == 0):
                            test_lib.test()
                    test_lib.test()
                    total = triplec.rel_num
                else:
                    for rel in range(total):
                        ph, pt, pr = triplec.getbatch(config.batch_size)
                        res = test_step(ph, pt, pr)
                        triplec.getThreshold(res)
                    triplec.showThreshold()
                    triplec.init = False
                    triplec.testFlag = True
                    triplec.batchId = 0
                    for rel in range(total):
                        ph, pt, pr = triplec.getbatch(config.batch_size)
                        res = test_step(ph, pt, pr)
                        triplec.tripleClassify(res)
