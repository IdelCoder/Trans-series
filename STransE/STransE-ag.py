...
class STransEagModel(object):
    ...
    def __init__(self, config, ent_init = None, rel_init = None):
        ...
        with tf.name_scope('lookup_embeddings'):
            ...
            pos_h_a_e = tf.reshape(tf.nn.embedding_lookup(self.ent_embeddings, self.pos_h_a), [-1, size, 1])
            pos_t_a_e = tf.reshape(tf.nn.embedding_lookup(self.ent_embeddings, self.pos_t_a), [-1, size, 1])
            pos_r_a_e = tf.reshape(tf.nn.embedding_lookup(self.rel_embeddings, self.pos_r_a), [-1, size])			
            ...
            pos_h_a_matrix = tf.reshape(tf.nn.embedding_lookup(self.rel_h_matrix, self.pos_r_a), [-1, size, size])
            pos_t_a_matrix = tf.reshape(tf.nn.embedding_lookup(self.rel_t_matrix, self.pos_r_a), [-1, size, size])
            ...
            pos_h_a_e = tf.nn.l2_normalize(tf.reshape(tf.matmul(pos_h_a_matrix, pos_h_a_e), [-1, size]), 1)
            pos_t_a_e = tf.nn.l2_normalize(tf.reshape(tf.matmul(pos_t_a_matrix, pos_t_a_e), [-1, size]), 1)
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            with tf.variable_scope("model", reuse=None, initializer = initializer):
                trainModel = STransEagModel(config = config)
            ...
            if (config.loadFromData):
                saver.restore(sess, './model/STransEagModel.vec')
            ...
            if not config.testFlag:
                for times in range(config.trainTimes):
                    ...
                    print times
                    print res
                saver.save(sess, './model/STransEagModel.vec')
            else:
