...
int relationTotal;
int entityTotal;
int testTotal, tripleTotal, trainTotal, validTotal = 0;
...
// 1-to-1, 1-to-N, N-to-1, N-to-N, 从下标1开始
int nntotal[5];
...
void init() {
    ...
    int total_lef = 0;
    int total_rig = 0;
    // type_constrain.txt第一行1345表示关系的数量
    // 接下来每两行就表示关系的头实体和关系的尾实体
    ...
    for (int i = 0; i < relationTotal; i++) {
        ...
        for (int j = 0; j < tot; j++) {
            // head_type记录关系的头实体和尾实体
            // 使用total_lef是记录每个关系头实体和尾实体的索引
            // head_lef[rel]和head_rig[rel]记录了关系的头实体的开始和结束位置
...
void test() {
    ...
    for (int i = 0; i <=0; i++) {
        ...
        printf("mean_rank_raw %f \n", ((l_rank[i] / testTotal) + (r_rank[i]/testTotal)) / 2);
        printf("mean_rank_filter %f \n", (l_filter_rank[i] / testTotal + r_filter_rank[i]/testTotal) / 2);
        printf("hits10_raw %f \n", (l10_tot[i] / testTotal + r10_tot[i] / testTotal)/2);
        printf("hits10_filter %f \n", (l10_filter_tot[i] / testTotal + r10_filter_tot[i] / testTotal)/2);
        printf("hits3_raw %f \n", (l3_tot[i] / testTotal + r3_tot[i] / testTotal)/2);
        printf("hits3_filter %f \n", (l3_filter_tot[i] / testTotal + r3_filter_tot[i] / testTotal)/2);
        printf("hits1_raw %f \n", (l1_tot[i] / testTotal + r1_tot[i] / testTotal)/2);
        printf("hits1_filter %f \n", (l1_filter_tot[i] / testTotal + r1_filter_tot[i] / testTotal)/2);
    }
