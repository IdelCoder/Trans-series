...
class TransMposModel(object):
    ...
    def __init__(self, config):
        ...
        if config.L1_flag:
            pos = tf.multiply(tf.reduce_sum(abs(pos_h_e + pos_r_e - pos_t_e), 1, keep_dims = True), pos_map_degree)
            neg = tf.multiply(tf.reduce_sum(abs(neg_h_e + neg_r_e - neg_t_e), 1, keep_dims = True), neg_map_degree)
            self.predict = pos
            pos_a = tf.multiply(tf.reduce_sum(abs(pos_h_a_e + pos_r_a_e - pos_t_a_e), 1, keep_dims=True), pos_a_map_degree)
        else:
            pos = tf.multiply(tf.reduce_sum((pos_h_e + pos_r_e - pos_t_e) ** 2, 1, keep_dims = True), pos_map_degree)
            neg = tf.multiply(tf.reduce_sum((neg_h_e + neg_r_e - neg_t_e) ** 2, 1, keep_dims = True), neg_map_degree)
            pos_a = tf.multiply(tf.reduce_sum((pos_h_a_e + pos_r_a_e - pos_t_a_e) ** 2, 1, keep_dims = True), pos_a_map_degree)
            self.predict = pos
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            with tf.variable_scope("model", reuse=None, initializer = initializer):
                trainModel = TransMposModel(config = config)
            ...
            if (config.loadFromData):
                saver.restore(sess, './model/transMposmodel.vec')
            ...
            if not config.testFlag:
                for times in range(config.trainTimes):
                    ...
                    print times
                    print res
                saver.save(sess, './model/transMposmodel.vec')
            else:
