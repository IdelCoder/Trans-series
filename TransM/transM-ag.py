...
map_degree = dict()
dir_path = "../data/FB15K/"
with open(dir_path + 'mdegree.txt', "r") as f:
	cf = csv.reader(f, delimiter=' ')
	for row in cf:
		map_degree[int(row[0])] = []
		map_degree[int(row[0])].append(float(row[1]))
		map_degree[int(row[0])].append(float(row[2]))
...
class TransMagModel(object):
    ...
    def __init__(self, config):
        ...
        with tf.name_scope("embedding"):
            ...
            mapping_degree = np.zeros([relation_total], dtype = np.float32)
			
            for i in range(relation_total):
                mapping_degree[i] = 1.0 / (math.log10(map_degree[i][0] + map_degree[i][1]))
            self.mapping_degree = tf.constant(mapping_degree, name="mapping_degree")


            pos_h_e = tf.nn.embedding_lookup(self.ent_embeddings, self.pos_h)
            pos_t_e = tf.nn.embedding_lookup(self.ent_embeddings, self.pos_t)
            pos_r_e = tf.nn.embedding_lookup(self.rel_embeddings, self.pos_r)
            pos_map_degree = tf.reshape(tf.nn.embedding_lookup(self.mapping_degree, self.pos_r), [-1,1])
            neg_h_e = tf.nn.embedding_lookup(self.ent_embeddings, self.neg_h)
            neg_t_e = tf.nn.embedding_lookup(self.ent_embeddings, self.neg_t)
            neg_r_e = tf.nn.embedding_lookup(self.rel_embeddings, self.neg_r)
            neg_map_degree = tf.reshape(tf.nn.embedding_lookup(self.mapping_degree, self.neg_r),[-1,1])
            ...
            pos_a_map_degree = tf.reshape(tf.nn.embedding_lookup(self.mapping_degree, self.pos_r_a), [-1,1])
            

            

        if config.L1_flag:
            pos = tf.multiply(tf.reduce_sum(abs(pos_h_e + pos_r_e - pos_t_e), 1, keepdims = True), pos_map_degree)
            neg = tf.multiply(tf.reduce_sum(abs(neg_h_e + neg_r_e - neg_t_e), 1, keepdims = True), neg_map_degree)
            h_ag = tf.reduce_sum(abs(pos_h_e - pos_h_a_e), 1, keepdims=True)
            t_ag = tf.reduce_sum(abs(pos_t_e - pos_t_a_e), 1, keepdims=True)
            self.predict = pos
        else:
            pos = tf.multiply(tf.reduce_sum((pos_h_e + pos_r_e - pos_t_e) ** 2, 1, keepdims = True), pos_map_degree)
            neg = tf.multiply(tf.reduce_sum((neg_h_e + neg_r_e - neg_t_e) ** 2, 1, keepdims = True), neg_map_degree)
            h_ag = tf.reduce_sum((pos_h_e - pos_h_a_e) ** 2, 1, keepdims=True)
            t_ag = tf.reduce_sum((pos_t_e - pos_t_a_e) ** 2, 1, keepdims=True)
            self.predict = pos

        with tf.name_scope("output"):
            self.loss = tf.reduce_sum(tf.maximum(pos - neg + margin, 0)) + tf.reduce_sum(tf.maximum(h_ag + t_ag - beta, 0))
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            with tf.variable_scope("model", reuse=None, initializer = initializer):
                trainModel = TransMagModel(config = config)
            ...
            if (config.loadFromData):
                saver.restore(sess, './model/transMagmodel.vec')
            ...
            if not config.testFlag:
                for times in range(config.trainTimes):
                    ...
                    print times
                    print res
                saver.save(sess, './model/transMagmodel.vec')
            else:
