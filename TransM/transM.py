...
class TransMModel(object):
    ...
    def __init__(self, config):
        ...
        with tf.name_scope("embedding"):
            ...
            mapping_degree = np.ones([relation_total], dtype = np.float32)
			
            for i in range(relation_total):
                # print("relation is: {}, tph is: {}, hpt is: {}".format(i, map_degree[i][0], map_degree[i][1]))
                mapping_degree[i] = 1.0 / (math.log(map_degree[i][0] + map_degree[i][1], 10))
                # print("mapping_degree is: {}".format(mapping_degree[i]))
        ...
        if config.L1_flag:
            ...
            self.result = tf.reduce_sum(abs(pos_h_e + pos_r_e - pos_t_e), 1, keepdims = True)
            self.pos_map_degree = pos_map_degree
            self.predict = pos
        else:
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            with tf.variable_scope("model", reuse=None, initializer = initializer):
                trainModel = TransMModel(config = config)
            ...
            if (config.loadFromData):
                saver.restore(sess, './model/transMmodel.vec')
            ...
            def train_step(pos_h_batch, pos_t_batch, pos_r_batch, neg_h_batch, neg_t_batch, neg_r_batch):
                ...
                _, step, loss, pos_map_degree = sess.run(
                    [train_op, global_step, trainModel.loss, trainModel.pos_map_degree], feed_dict)
                # print(pos_map_degree)
                return loss
            ...
            if not config.testFlag:
                for times in range(config.trainTimes):
                    ...
                    print times
                    print res
                saver.save(sess, './model/transMmodel.vec')
            else:
