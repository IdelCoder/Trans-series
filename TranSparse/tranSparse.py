...
rel_ratio = dict()
dir_path = "../data/FB15K/"
with open(dir_path + 'rel_ratio.txt', "r") as f:
    cf = csv.reader(f, delimiter=' ')
    for row in cf:
        rel_ratio[int(row[0])] = []
        # share ratio
        rel_ratio[int(row[0])].append(float(row[1]))
        # separate head ratio
        rel_ratio[int(row[0])].append(float(row[2]))
        # separate tail ratio
        rel_ratio[int(row[0])].append(float(row[3]))

def normalize_mat(rel_matrix, sizeR, sizeE):
	temp_mat = rel_matrix.reshape(sizeR, sizeE)
	square = temp_mat * temp_mat
	square_sum = square.sum(axis=1, keepdims=True)
	res_mat = np.nan_to_num(np.sqrt(square / square_sum))
	return np.reshape(res_mat, sizeR * sizeE)
...
class TranSparseShareModel(object):
    ...
    def __init__(self, config, ent_init = None, rel_init = None):
        ...
        with tf.name_scope("embedding"):
            ...
            for i in range(relation_total):
                ratio = 1-(1-config.thetamin)*rel_ratio[i][0]
                for j in range(sizeR * sizeE):
                    if random.random() <= ratio:
                        rel_matrix[i][j] = 1.0
                    else:
                        rel_matrix[i][j] = 0
                rel_matrix[i] = normalize_mat(rel_matrix[i], sizeR, sizeE)
...
class TranSparseSepModel(object):
    ...
    def __init__(self, config, ent_init = None, rel_init = None):
        ...
        with tf.name_scope("embedding"):
            ...
            rel_h_matrix = np.zeros([relation_total, sizeR * sizeE], dtype = np.float32)
            for i in range(relation_total):
                ratio = 1-(1-config.thetamin)*rel_ratio[i][1]
                for j in range(sizeR * sizeE):
                    if random.random() <= ratio:
                        rel_h_matrix[i][j] = 1.0
                    else:
                        rel_h_matrix[i][j] = 0
            self.rel_h_matrix = tf.Variable(rel_h_matrix, name = "rel_h_matrix")

            rel_t_matrix = np.zeros([relation_total, sizeR * sizeE], dtype = np.float32)
            for i in range(relation_total):
                ratio = 1-(1-config.thetamin)*rel_ratio[i][2]
                for j in range(sizeR * sizeE):
                    if random.random() <= ratio:
                        rel_t_matrix[i][j] = 1.0
                    else:
                        rel_t_matrix[i][j] = 0
        ...
        with tf.name_scope('lookup_embeddings'):
            ...
            pos_h_matrix = tf.reshape(tf.nn.embedding_lookup(self.rel_h_matrix, self.pos_r), [-1, sizeR, sizeE])
            pos_t_matrix = tf.reshape(tf.nn.embedding_lookup(self.rel_t_matrix, self.pos_r), [-1, sizeR, sizeE])
            neg_h_matrix = tf.reshape(tf.nn.embedding_lookup(self.rel_h_matrix, self.neg_r), [-1, sizeR, sizeE])
            neg_t_matrix = tf.reshape(tf.nn.embedding_lookup(self.rel_t_matrix, self.neg_r), [-1, sizeR, sizeE])

            pos_h_e = tf.nn.l2_normalize(tf.reshape(tf.matmul(pos_h_matrix, pos_h_e), [-1, sizeR]), 1)
            pos_t_e = tf.nn.l2_normalize(tf.reshape(tf.matmul(pos_t_matrix, pos_t_e), [-1, sizeR]), 1)
            neg_h_e = tf.nn.l2_normalize(tf.reshape(tf.matmul(neg_h_matrix, neg_h_e), [-1, sizeR]), 1)
            neg_t_e = tf.nn.l2_normalize(tf.reshape(tf.matmul(neg_t_matrix, neg_t_e), [-1, sizeR]), 1)
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            with tf.variable_scope("model", reuse=None, initializer = initializer):
                if config.shareFlag:
                    trainModel = TranSparseShareModel(config = config, ent_init = config.ent_init, rel_init = config.rel_init)
                else:
                    trainModel = TranSparseSepModel(config = config, ent_init = config.ent_init, rel_init = config.rel_init)
            ...
            if (config.loadFromData):
                if config.shareFlag:
                    saver.restore(sess, './model/tranSparsesharemodel.vec')
                else:
                    saver.restore(sess, './model/tranSparsesepmodel.vec')
            ...
            if not config.testFlag:
                for times in range(config.trainTimes):
                    ...
                    print res
                if config.shareFlag:
                    saver.save(sess, './model/tranSparsesharemodel.vec')
                else:
                    saver.save(sess, './model/tranSparsesepmodel.vec')
            else:
