...
class TransRposModel(object):
    ...
    def __init__(self, config, ent_init = None, rel_init = None):
        ...
        with tf.name_scope("read_inputs"):
            ...
            self.pos_h_a = tf.placeholder(tf.int32, [batch_size])
            self.pos_t_a = tf.placeholder(tf.int32, [batch_size])
            self.pos_r_a = tf.placeholder(tf.int32, [batch_size])
        ...
        with tf.name_scope('lookup_embeddings'):
            ...
            pos_h_a_e = tf.reshape(tf.nn.embedding_lookup(self.ent_embeddings, self.pos_h_a), [-1, sizeE, 1])
            pos_t_a_e = tf.reshape(tf.nn.embedding_lookup(self.ent_embeddings, self.pos_t_a), [-1, sizeE, 1])
            pos_r_a_e = tf.reshape(tf.nn.embedding_lookup(self.rel_embeddings, self.pos_r_a), [-1, sizeR])
            pos_a_matrix = tf.reshape(tf.nn.embedding_lookup(self.rel_matrix, self.pos_r_a), [-1, sizeR, sizeE])
            ...
            pos_h_a_e = tf.nn.l2_normalize(tf.reshape(tf.matmul(pos_a_matrix, pos_h_a_e), [-1, sizeR]), 1)
            pos_t_a_e = tf.nn.l2_normalize(tf.reshape(tf.matmul(pos_a_matrix, pos_t_a_e), [-1, sizeR]), 1)
        ...
        with tf.name_scope("output"):
            self.loss = tf.reduce_sum(tf.maximum(pos - neg + margin, 0)) + tf.reduce_sum(
                tf.maximum(abs(pos - pos_a) - beta, 0)
            )
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            with tf.variable_scope("model", reuse=None, initializer = initializer):
                trainModel = TransRposModel(config = config, ent_init = config.ent_init, rel_init = config.rel_init)
            ...
            if (config.loadFromData):
                saver.restore(sess, './model/transRposmodel.vec')
            ...
            if not config.testFlag:
                for times in range(config.trainTimes):
                    ...
                    print times
                    print res
                saver.save(sess, './model/transRposmodel.vec')
            else:
