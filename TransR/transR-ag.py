    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            with tf.variable_scope("model", reuse=None, initializer = initializer):
                trainModel = TransRagModel(config = config, ent_init = config.ent_init, rel_init = config.rel_init)
            ...
            if (config.loadFromData):
                saver.restore(sess, './model/transRagmodel.vec')
            ...
            if not config.testFlag:
                for times in range(config.trainTimes):
                    ...
                    print times
                    print res
                saver.save(sess, './model/transRagmodel.vec')
            else:
