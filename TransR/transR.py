...
class Config(object):
	...
	def __init__(self):
		...
		'''
		# self.rel_init = "../TransE/embedding/ent_embeddings.txt"
		# self.ent_init = "../TransE/embedding/rel_embeddings.txt"
		self.rel_init = None
		self.ent_init = None
	...
	with tf.Graph().as_default():
		...
		with sess.as_default():
			...
			if (config.loadFromData):
				saver.restore(sess, './model/transRmodel.vec')
			...
			if not config.testFlag:
				for times in range(config.trainTimes):
					...
					print times
					print res
				saver.save(sess, './model/transRmodel.vec')
			else:
