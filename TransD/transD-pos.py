...
class Config(object):
    ...
    def __init__(self):
        ...
        self.entity = 0
        self.relation = 0
        self.trainTimes = 5000
        self.margin = 1.0
        self.beta = 1.0

class TransDposModel(object):
    ...
    def __init__(self, config):
        ...
        with tf.name_scope("embedding"):
            ...
            pos_h_a_t = tf.nn.embedding_lookup(self.ent_transfer, self.pos_h_a)
            pos_t_a_t = tf.nn.embedding_lookup(self.ent_transfer, self.pos_t_a)
            pos_r_a_t = tf.nn.embedding_lookup(self.rel_transfer, self.pos_r_a)
            ...
            pos_h_a_e = self.calc(pos_h_a_e, pos_h_a_t, pos_r_a_t)
            pos_t_a_e = self.calc(pos_t_a_e, pos_t_a_t, pos_r_a_t)
        ...
        with tf.name_scope("output"):
            self.loss = tf.reduce_sum(tf.maximum(pos - neg + margin, 0)) + tf.reduce_sum(tf.maximum(
                abs(pos - pos_a) - beta, 0
            ))
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            with tf.variable_scope("model", reuse=None, initializer = initializer):
                trainModel = TransDposModel(config = config)
            ...
            if (config.loadFromData):
                saver.restore(sess, './model/transDposmodel.vec')
            ...
            if not config.testFlag:
                for times in range(config.trainTimes):
                    ...
                    print times
                    print res
                saver.save(sess, './model/transDposmodel.vec')
            else:
